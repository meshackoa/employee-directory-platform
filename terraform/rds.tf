...
resource "aws_db_subnet_group" "main" {
  name       = "employee-platform-db-subnet-${var.environment}"
...
resource "aws_security_group" "rds" {
  name        = "employee-platform-rds-sg-${var.environment}"
...
resource "aws_db_instance" "main" {
  identifier     = "employee-platform-db-${var.environment}"
  engine         = "postgres"
  engine_version = "15.7"
  ...
  final_snapshot_identifier = var.environment == "prod" ? "employee-platform-db-final-${var.environment}" : null

  backup_retention_period = var.environment == "prod" ? 7 : 1

  tags = {
    Name        = "employee-platform-db-${var.environment}"
    Environment = var.environment
  }
}
